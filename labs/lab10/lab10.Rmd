...
A gives the product of the grid and stores it in variable z

B stores the max value of Y in i

```{r}
bernoulliMax <- function(d,x)
{
  n = length(d)
  x^sum(d)*(1-x)^(n-sum(d))
  ...
  y = c(3,3,4,3,4,5,5,4)
  plot(seq(0,1,length.out=8),bernoulliMax(seq(0,1,length.out=8),y),type="b",lwd=2)
```
...
y = c(4,6,7,6,5)
f = function(x,param) log(dpois(x,lambda = param))
mymaxlik(lfun=f,x=y,param = seq(0,10,length=1000))
```
...
myNRML(x0 = 5.6, delta = 0.00001, llik=function(x) log(dpois(12,x)*dpois(10,x)),xrange = c(0,15), parameter = "lambda")
```
...
mymlnorm(x=y,mu = seq(10,15,length=1000),sig = seq(0.1,4,length=1000))
```
...
mymlbeta(sample(rbeta(30,3,4),30),alpha=seq(1,8,length=100),beta = seq(2,8,length=100))
```
...
```{r}
MATH4753BERN0021::mymlbeta(sample(rbeta(30,3,4),30),alpha=seq(1,8,length=100),beta = seq(2,8,length=100))
```
