...
renderPlot({
 ...
 gp = ggplot(spruce.df, aes(x = BHDiameter, y = Height), fill="#69b3a2") + geom_point(aes(col = Height)) 
 
 gt = ggplot(spruce.df, aes(x = BHDiameter, y = Height)) + geom_line()
 
  gs = ggplot(spruce.df, aes(x = BHDiameter, y = Height)) + geom_smooth(method = "lm")
  
 if(input$plotg == "trend") print(gt)
 
 if(input$plotg == "points")   print(gp)
 
 if(input$plotg == "segments")   print(gs)
})
```
